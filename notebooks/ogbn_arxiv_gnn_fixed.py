# -*- coding: utf-8 -*-
"""OGBN_Arxiv_GNN_FIXED (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1P297N88nJyK5H_FBp72i-V1A5_PoCFjF
...
"""**## Feature-Only Baseline: MLP**"""

print("Features:", data.x.shape)
print("Labels:", data.y.shape)
print("Train nodes:", train_idx.shape)
print("Validation nodes:", valid_idx.shape)
print("Test nodes:", test_idx.shape)
...
# MLP configuration
mlp_input_dim = data.x.shape[1]   # 128 features
mlp_hidden_dim = 256
mlp_output_dim = int(data.y.max().item()) + 1
...
print(mlp_model)
print("Input features:", mlp_input_dim)
print("Output classes:", mlp_output_dim)
...
# --- FIX: Move data and indices to the GPU device ---
data = data.to(device)
...
for epoch in range(mlp_epochs):
    ...
    # IMPORTANT:
    # MLP uses ONLY node features.
    # No edge_index is used.
    output = mlp_model(data.x)
    ...
    with torch.no_grad():
        val_output = mlp_model(data.x)
        val_pred = val_output[valid_idx].argmax(dim=1)

        val_acc = (
            val_pred == data.y[valid_idx].squeeze()
        ).float().mean().item()
...
print(f"Best Validation Accuracy: {mlp_best_val_acc:.4f}")
